    ...
    <div class="max-w-4xl mx-auto bg-white rounded-lg shadow-md overflow-hidden">
      ...
      <div class="p-6">
        ...
        <div v-if="currentStep === 2">
          <div class="space-y-6">
            <div>
              ...
              <form class="my-4">
                ...
                <div class="relative">
                  ...
                  <input id="zipcode" v-model="formData.zipcode" type="text"
                    ...
                    :class="{ 'border-red-500': errors.zipcode }" placeholder="Enter your zip code" required />
              ...
              <div class="grid grid-cols-2 gap-4">
                <div v-for="(location, index) in available_locations" :key="index"
                  class="border rounded-lg p-4 cursor-pointer" :class="{
                    'bg-primary text-white': formData.selectedLocation === index && formData.location_id !== '',
                    'hover:bg-gray-100': !(formData.selectedLocation === index && formData.location_id !== ''),
              ...
              <!-- Add this after the location selection grid in Step 2 -->
              <div v-if="formData.location_id && available_groomers.length > 0" class="mt-6">
                <h3 class="text-lg font-medium mb-3">Select Your Groomer</h3>
                <p class="text-sm text-gray-500 mb-3">Choose a professional groomer for your pet's appointment</p>
 
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  <div v-for="groomer in available_groomers" :key="groomer.id"
                    class="border rounded-lg p-4 cursor-pointer" :class="{
                      'bg-primary text-white': formData.groomer_id === groomer.id,
                      'hover:bg-gray-100': formData.groomer_id !== groomer.id,
                    }" @click="formData.groomer_id = groomer.id; errors.groomer_id = ''">
                    <h4 class="font-medium">{{ groomer.name }}</h4>
                    <p class="text-sm"
                      :class="{ 'text-gray-500': formData.groomer_id !== groomer.id, 'text-white': formData.groomer_id === groomer.id }">
                      {{ groomer.phone_number || 'General Grooming' }}
                    </p>
                ...
                <p v-if="errors.groomer_id" class="mt-1 text-sm text-red-600">
                  {{ errors.groomer_id }}
                </p>
              </div>
            </div>
        ...
        <div v-if="currentStep === 3">
          <div class="space-y-6">
            <div>
              <vue-cal class="vuecal--rounded-theme custom-blue-theme !w-full" xsmall hide-view-selector :time="false"
                active-view="month" :disable-views="['week']" style="width: 270px; height: 300px"
                ...
                <div class="mt-6 sm:mt-8 gap-3 lg:flex lg:items-start">
                    ...
                    <div class="mb-6 grid grid-cols-2 gap-4">
                      ...
                      <div>
                        <label for="card-expiration-input"
                          class="mb-2 block text-sm font-medium text-gray-900 dark:text-white">Card expiration* </label>
                        <div class="relative">
                  ...
                  <div class="mt-6 grow sm:mt-8 lg:mt-0">
                      ...
                      <div class="space-y-2">
                        <dl class="flex items-center justify-between gap-4">
                          ...
                          <dd class="text-base font-medium text-gray-900 dark:text-white">
                            ${{ formData.service_price.prices[0].amount }}</dd>
                        </dl>
                    ...
                    <div class="mt-6 flex items-center justify-center gap-4">
                      <img class="h-6 w-6 w-auto dark:hidden"
            ...
            <div class="space-y-6 text-[15px]">
              ...
              <div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 text-gray-700">
                  ...
                  <p>
                    ...
                    {{
                      formData.selectedTimeSlot !== null
                        ? available_slots[formData.selectedTimeSlot].start_time + "-" +
                        available_slots[formData.selectedTimeSlot].end_time
                        : ""
                    }}
                  </p>
                  <p>
                  ...
                  <!-- New groomer information -->
                  <p v-if="formData.groomer_id">
                    <span class="font-medium">Groomer:</span>
                    {{ getGroomerName(formData.groomer_id) }}
                  </p>
                </div>
              </div>
...
// Validate step 2 (Location and Groomer Selection)
const validateStep2 = () => {
  // Clear previous errors
  errors.selectedLocation = "";
  errors.zipcode = "";
  ...
  // Validate groomer selection if groomers are available
  if (formData.location_id !== "" && available_groomers.value.length > 0 && !formData.groomer_id) {
    errors.groomer_id = "Please select a groomer";
    isValid = false;
  }
...
// Function to select location and fetch groomers
const selectLocation = async (location, index) => {
  formData.selectedLocation = index;
  formData.location_id = location.id;
  formData.location_address = location.address[0].address;
 
  // Clear previous groomer selection
  formData.groomer_id = "";
 
  // Fetch available groomers for this location
  try {
    const response = await axios.get(`http://localhost:9000/store/groomer?location_id=${location.id}`, {
      headers: {
    ...
    available_groomers.value = response.data.groomers || [];
 
    if (available_groomers.value.length === 0) {
      errorMessage.value = "No groomers available at this location";
      setTimeout(() => {
        errorMessage.value = "";
      }, 3000);
    }
    ...
    errorMessage.value = "Failed to load groomers for this location";
    setTimeout(() => {
      errorMessage.value = "";
    }, 3000);
    available_groomers.value = [];
  }
};
 
// Helper function to get groomer name by ID for display in review
const getGroomerName = (groomerId) => {
  const groomer = available_groomers.value.find(g => g.id === groomerId);
  return groomer ? groomer.name : 'Not specified';
};
 
// Helper function to get groomer specialty by ID
const getGroomerSpecialty = (groomerId) => {
  const groomer = available_groomers.value.find(g => g.id === groomerId);
  return groomer && groomer.speciality ? groomer.speciality : 'General Grooming';
};
// ======================================================
...
const submitForm = async () => {
  ...
  const body = {
    ...
    groomer_id: formData.groomer_id, // Add the groomer ID to the booking
    location_address: {
